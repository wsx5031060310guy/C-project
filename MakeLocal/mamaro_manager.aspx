<%@ Page Language="C#" AutoEventWireup="true" CodeFile="mamaro_manager.aspx.cs" Inherits="mamaro_manager" %>
...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
    ...
    <title>mamaro Manager for customer</title>
...
<body>
    <div class="container">
		<div class="top">
			<h1 id="title" class="hidden"><span id="logo">mamaro</span></h1>
		</div>
        ...
        $(document).ready(function () {
            ...
            $("#loginbutton").click(function () {
                ...
                if ($('#username').val().replace("'", "").replace('"', "").replace("`", "").replace(/\s/g, '').trim() != "" && $('#password').val().replace("'", "").replace('"', "").replace("`", "").replace(/\s/g, '').trim() != "") {
                    $.ajax({
                        type: "POST",
                        url: "mamaro_manager.aspx/check_login",
                        ...
                        success: function (result) {
                            ...
                            alert(result.d);
                            var str = "<%= Session["manager_page"]%>".toString();

                                    if (str != "" || str != null) {
                                        window.location.href = "mamaro_select.aspx";
                                    }
                                },
