...
public partial class main_guest : System.Web.UI.Page
{
...
//////////////////
                    ...
                    //addstr += " inner join user_login as b on b.id=a.uid where 1=1 and a.type='0'";
                    ...
                    //string addstr = " and a.id not in (select top (@FromRange-1) a.id from status_messages as a WITH (INDEX(IX_status_messages),INDEX(IX_status_messages_1))";
                    //addstr += " inner join user_login as b on b.id=a.uid where 1=1 and a.type='0'";
                    ...
                    //sql_place.SelectCommand = "declare @FromRange int declare @ToRange int set @FromRange ='1' set @ToRange ='" + Convert.ToInt32(Session["top_count"].ToString()) + "'";
                    //sql_place.SelectCommand += " select top(@ToRange-@FromRange+1) a.place_lat,a.place_lng,a.id,a.type,a.message_type,a.place,a.message,a.year,a.month,a.day,a.hour,a.minute,a.second,b.username,b.photo,a.uid ";
                    //sql_place.SelectCommand += "from status_messages as a WITH (INDEX(IX_status_messages),INDEX(IX_status_messages_1))";
                    //sql_place.SelectCommand += " inner join user_login as b on b.id=a.uid where 1=1 and a.type='0'";
                    if (Session["message_type"] != null)
                    {
                        if (Session["message_type"].ToString() != "")
                        {
...
$('.image-link').magnificPopup({
            ...
            for (int i = 0; i < ict.Count; i++)
            {
                ...
                if (ict.Table.Rows[i]["message"].ToString().Length < 37)
                {
                    result_res += main_guest.ConvertUrlsToLinks(ict.Table.Rows[i]["message"].ToString());
                }
                else
                {
                ...
                result_res += "<span style='word-break:break-all;over-flow:hidden;'>" + main_guest.ConvertUrlsToLinks(ict.Table.Rows[i]["message"].ToString()) + "</span>";
                ...
                result_res += "</div>";
                result_res += "<div>";
                result_res += "<span style='word-break:break-all;over-flow:hidden;'>" + main_guest.ConvertUrlsToLinks_DIV(ict.Table.Rows[i]["message"].ToString()) + "</span>";
                result_res += "</div>";
                result_res += "</td>";
                result_res += "</tr>";
                //poster images
